 ...
 * @param value 身份证号码组成的数组
 */
const isTrueValidateIdCard = (value: string[]) => {
  // 加权因子
  ...
  for (let i = 0; i < 17; i++) {
    sum += Wi[i] * Number(value[i]) // 加权求和
  }
  const valCodePosition = sum % 11 // 得到验证码所位置

  const lastVal = value[17]
  if (lastVal === 'X') {
    if (lastVal !== ValideCode[valCodePosition]) {
      return false
    } else {
      return true
    }
  } else {
    if (Number(lastVal) !== ValideCode[valCodePosition]) {
      return false
    } else {
      return true
    }
